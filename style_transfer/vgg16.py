"""Implementation of our own architecture of the VGG16 network. """
...
class Vgg16(nn.Module):
    def __init__(self, mode='generative_st'):
        ...
        self.mode = mode
        features = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features #import of the already built-in VGG16 network.
        self.feat = features
    ...
    def forward(self, x):
        ...
        for i in range(len(self.relu_names)):
            ...
            if (self.mode == 'generative_st') and (len(feature_maps) == len(self.extracted_fmaps)):
                return tuple(feature_maps) #we return the feature_maps as is for the generative approach.
        
        feature_maps[-2], feature_maps[-1] = feature_maps[-1], feature_maps[-2]
        #after this line, the order of the feature maps the order is as follows for descriptive_st: ['1_1', '2_1', '3_1', '4_1', '5_1, '4_2'].
        #The first five items correspond to the style extraction, the last one corresponds to content extraction. This order allows us to use the index [-1] for content extraction.
